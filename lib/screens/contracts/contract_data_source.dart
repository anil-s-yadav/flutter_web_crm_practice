...
class ContractDataSource extends DataGridSource {
  ...
  String _getDurationText(ContractModel contract) {
    final days = contract.guaranteeEndDate.difference(contract.placementDate).inDays;
  ...
  void _buildDataGridRows() {
    _dataGridRows = _contracts.map<DataGridRow>((contract) {
      return DataGridRow(
        cells: [
          ...
          DataGridCell<String>(
            columnName: 'date',
            value: DateFormat('MMM dd, yyyy').format(contract.placementDate),
          ),
          DataGridCell<String>(
            columnName: 'duration',
          ...
          DataGridCell<String>(
            ...
            value: DateFormat('MMM dd, yyyy').format(contract.guaranteeEndDate),
          ),
          ...
          DataGridCell<ContractModel>(columnName: 'actions', value: contract),
        ],
      );
    }).toList();
  }
  ...
  int compare(DataGridRow? a, DataGridRow? b, SortColumnDetails sortColumn) {
    if (sortColumn.name == 'details' || sortColumn.name == 'actions') {
      final value1 = a?.getCells().firstWhere((c) => c.columnName == sortColumn.name).value as ContractModel?;
      final value2 = b?.getCells().firstWhere((c) => c.columnName == sortColumn.name).value as ContractModel?;
      ...
      final value1 = a?.getCells().firstWhere((c) => c.columnName == sortColumn.name).value as ContractStatus?;
      final value2 = b?.getCells().firstWhere((c) => c.columnName == sortColumn.name).value as ContractStatus?;
      if (value1 == null || value2 == null) return 0;
      if (sortColumn.sortDirection == DataGridSortDirection.ascending) {
      ...
      final value1 = a?.getCells().firstWhere((c) => c.columnName == sortColumn.name).value as PaymentStatus?;
      final value2 = b?.getCells().firstWhere((c) => c.columnName == sortColumn.name).value as PaymentStatus?;
      if (value1 == null || value2 == null) return 0;
      if (sortColumn.sortDirection == DataGridSortDirection.ascending) {
        return value1.name.compareTo(value2.name);
      } else {
  ...
  DataGridRowAdapter? buildRow(DataGridRow row) {
    final contract = row.getCells().firstWhere((c) => c.columnName == 'details').value as ContractModel;
  ...
  Color _getPaymentStatusColor(PaymentStatus status) {
    switch (status) {
      case PaymentStatus.pending: return AppColors.standardBlue;
      case PaymentStatus.partial: return AppColors.urgentAmber;
      case PaymentStatus.paid: return AppColors.successGreen;
      case PaymentStatus.overdue: return AppColors.criticalRed;
    }
  }

  Color _getContractStatusColor(ContractStatus status) {
    switch (status) {
      case ContractStatus.pending: return AppColors.standardBlue;
      case ContractStatus.active: return AppColors.successGreen;
      case ContractStatus.completed: return AppColors.grey500;
      case ContractStatus.replaced: return AppColors.urgentAmber;
      case ContractStatus.cancelled: return AppColors.criticalRed;
    }
  }
}
